...
io.sockets.on("connection", socket => { // quando avviene una connessione
  ...
  socket.on("map", (map , pose) => { //prendo mappa dal robot e la echo al client
    socket.in(newroom).emit("mapecho", map , pose); 
  });
  ...
  //NEW
  socket.on("cancelGoalFromScript" , () =>{
  	socket.in(newroom).emit("cancelGoalFromServer");
  });
  //NEW
  socket.on("direction" , (arg1,arg2) =>{
	  socket.in(newroom).emit("directionrobot", arg1, arg2);  
  });

  socket.on("mapPosServer", (arg1 , arg2) => { //se mi disconnetto il broadcaster deve chiudere la connessione
    socket.in(newroom).emit("mapPosBroad", arg1, arg2); 
  });
  
  socket.on("feedbackFromBroad" , (res) =>{ //prendo risultato dal robot e lo echo al client
  	socket.in(newroom).emit("feedbackFromServer" , res);
    if(res == "Goal Raggiunto" || res == "Goal Fallito" || res == "Goal Rifiutato" || res == "Goal Cancellato"){
      socket.in(newroom).emit("enableButtons");
      socket.in(newroom).emit("cancelGP");
    }
  });
  
  //Riceve in input le coordinate punto dove devo disegnare
  socket.on("drawGPFromBroad", (x , y ) => { //se mi disconnetto il broadcaster deve chiudere la connessione
    socket.in(newroom).emit("drawGPFromServer", x , y); 
  });
