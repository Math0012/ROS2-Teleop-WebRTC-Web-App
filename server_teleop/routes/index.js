  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()){ //se ci sono errori ritorna errore
      return res.render("register", { result: "utente non soddisfa i requisiti minimi di lunghezza username "+process.env.MIN_LENGTH_USER_FIELD+" o i max "+process.env.MAX_LENGTH_USER_FIELD+
        " oppure i minimi di password "+process.env.MIN_LENGTH_PW_FIELD+" o i max "+process.env.MAX_LENGTH_PW_FIELD+" oppure la password non contiene almeno una minuscola, una maiuscola e un numero; oppure sbagliato a digitare gruppo (deve essere user, admin o group)", csrfToken: req.csrfToken()});
      //return res.status(422).json({errors: errors.array()});
    }
...
router.post('/register', registerValidate, csrfProtection, connectEnsureLogin.ensureLoggedIn(), function(req, res, next) {
  ...
  UserDetails.register({ username: req.body.username, group: req.body.group, robotids : [ 
    ...
    if(err){console.log(err); res.render("register", { result: "utente già registrato", csrfToken: req.csrfToken()})}
    else{
      console.log("utente registrato");
      res.render("register", { result: "utente registrato con successo", csrfToken: req.csrfToken()})
    }
    });
});
...
const deleteValidate = [ //qui valido e sanitizzo input del delete
  ...
  (req, res, next) => {
    let errors = validationResult(req); //salvo errori
    if (!errors.isEmpty()){ //se ci sono errori ritorna errore
      return res.render("deleteuser", { result: "nome utente non valido", csrfToken: req.csrfToken()});
      //return res.status(422).json({errors: errors.array()});
    }
    next(); //se no prosegui con prossimo middleware
   }
...
router.post('/deleteuser', deleteValidate, csrfProtection, connectEnsureLogin.ensureLoggedIn(), function(req, res, next) {
  ...
  UserDetails.findOne({
    ...
    if (err) {
      console.log(err); res.render("deleteuser", { result: err, csrfToken: req.csrfToken()});
    }
    else{
      if(user!==null){
        UserDetails.remove({
          ...
          if (err) {
              console.log(err); res.render("deleteuser", { result: "utente inesistente", csrfToken: req.csrfToken()});
          }
          else{
             console.log("utente eliminato");
             res.render("deleteuser", { result: "utente eliminato con successo", csrfToken: req.csrfToken()})
          }
        });
       }
      else{
        res.render("deleteuser", { result: "utente inesistente", csrfToken: req.csrfToken()});
      }
    }
  });
});
...
const viewpermissionValidate = [ //qui valido e sanitizzo input del delete
  ...
  (req, res, next) => {
    let errors = validationResult(req); //salvo errori
    if (!errors.isEmpty()){ //se ci sono errori ritorna errore
      return res.render("viewpermissions", { result: "nome utente non valido", csrfToken: req.csrfToken()});
      //return res.status(422).json({errors: errors.array()});
    }
    next(); //se no prosegui con prossimo middleware
   }
...
router.post('/viewpermissions', viewpermissionValidate, csrfProtection, connectEnsureLogin.ensureLoggedIn(), function(req, res, next) {
  ...
  UserDetails.findOne({
    ...
    if (err) {
      console.log(err); 
      res.render("viewpermissions", { result: err, robotlist: '', csrfToken: req.csrfToken()});
    }
    else{
     if(user!==null){
      let arr = [];
      ...
      if(arr.length==0){
        res.render("viewpermissions", { result: "Nessun permesso trovato", robotlist: '', csrfToken: req.csrfToken()})
      }
      else{
      res.render("viewpermissions", { result: "I permessi sono:", robotlist: arr, csrfToken: req.csrfToken()})
      }
     }
     else{
      res.render("viewpermissions", { result: "utente inesistente", robotlist: '', csrfToken: req.csrfToken()});
     }
    }
  });
});
...
const addpermissionValidate = [ //qui valido e sanitizzo input del delete
  ...
  (req, res, next) => {
    let errors = validationResult(req); //salvo errori
    if (!errors.isEmpty()){ //se ci sono errori ritorna errore
      return res.render("addpermission", { result: "nome utente o robotid non valido", csrfToken: req.csrfToken()});
      //return res.status(422).json({errors: errors.array()});
    }
    next(); //se no prosegui con prossimo middleware
   }
...
router.post('/addpermission', addpermissionValidate, csrfProtection, connectEnsureLogin.ensureLoggedIn(), function(req, res, next) {
  ...
  function(err, user) {
    if (err) {
      console.log(err); res.render("addpermission", { result: err, csrfToken: req.csrfToken()});
    }
    else{
      if(user!==null){
        UserDetails.update(
        ...
        function (err,user){
          if (err) {
            ...
            res.render("addpermission", { result: "permesso aggiunto con successo", csrfToken: req.csrfToken()})
          }
        });
      }
       else{
       res.render("addpermission", { result: "utente inesistente", csrfToken: req.csrfToken()});
       }
      }
  });
});
...
const removepermissionValidate = [ //qui valido e sanitizzo input del delete
  ...
  (req, res, next) => {
    let errors = validationResult(req); //salvo errori
    if (!errors.isEmpty()){ //se ci sono errori ritorna errore
      return res.render("removepermission", { result: "nome utente o robotid non valido", csrfToken: req.csrfToken()});
      //return res.status(422).json({errors: errors.array()});
    }
    next(); //se no prosegui con prossimo middleware
   }
...
router.post('/removepermission', removepermissionValidate, csrfProtection, connectEnsureLogin.ensureLoggedIn(), function(req, res, next) {
  ...
  function(err, user) {
    if (err) {
      console.log(err); res.render("removepermission", { result: err, csrfToken: req.csrfToken()});
    }
    else{
      if(user!==null){
        UserDetails.update(
        ...
        function (err,user){
          if (err) {
            ...
            res.render("removepermission", { result: "permesso rimosso con successo", csrfToken: req.csrfToken()})
          }
        });
      }
       else{
       res.render("removepermission", { result: "utente inesistente", csrfToken: req.csrfToken()});
       }
      }
  });
});
